...
def is_multicast_mac(mac_address):
    # Check if the MAC address is in the multicast range (01:00:5e:XX:XX or 33:33:XX:XX)
    return (
        mac_address.startswith("01:00:5e")
        or mac_address.startswith("33:33:")
        or mac_address.startswith("00:")
    )
...
def extract_addresses(pcapng_file):
    ...
    for packet in capture:
        ...
        if "wlan" in packet:
            ...
            if (
                hasattr(wlan_layer, "sa")
                and hasattr(wlan_layer, "da")
                and hasattr(wlan_layer, "ta")
                and hasattr(wlan_layer, "ra")
            ):
                ...
                if (
                    source_address == "ff:ff:ff:ff:ff:ff"
                    or destination_address == "ff:ff:ff:ff:ff:ff"
                    or receiver_address == "ff:ff:ff:ff:ff:ff"
                    or transmitter_address == "ff:ff:ff:ff:ff:ff"
                    or is_multicast_mac(source_address)
                    or is_multicast_mac(destination_address)
                    or is_multicast_mac(receiver_address)
                    or is_multicast_mac(transmitter_address)
                ):
                    continue
                if wlan_type == "2":
...
async def upload_pcap(pcapng_file: UploadFile = UploadFile(...)):
    ...
    return {
        "addresses": addresses,
        "compenents": component_info,
        "no_of_disconnected_graphs": no_of_disconnected_graphs,
    }
...
def load_first_section_of_csv(content):
    ...
    df = pd.read_csv(
        io.BytesIO(first_section), skipinitialspace=True
    )  # Use skipinitialspace to remove whitespace from keys
...
def load_second_section_of_csv(content):
    ...
    df = pd.read_csv(
        io.BytesIO(second_section), skipinitialspace=True
    )  # Use skipinitialspace to remove whitespace from keys
...
async def upload_and_analyze(file: UploadFile = File(...)):
    try:
        ...
        if file.filename.endswith(".csv"):
            ...
            first_section_analysis = [
                {str(k): str(v).strip() for k, v in row.items()}
                for row in first_section_analysis
            ]
            second_section_analysis = [
                {str(k): str(v).strip() for k, v in row.items()}
                for row in second_section_analysis
            ]
...
def execute_script(duration: int):
    script_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "scripts/first.sh"
    )
    try:
        subprocess.Popen(
            [
                ...
                "bash",
                "-c",
                f"{script_path} {duration}; exec bash",
            ]
        )
...
async def get_csv_file():
    ...
    csv_file = os.path.join("scripts/output", "capture.csv")
...
async def get_pcap_file():
    ...
    pcap_file = os.path.join("scripts/output", "capture")
    print(1)
...
def execute_attack(bssid: str, channel: int):
    script_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "scripts/second.sh"
    )
    print(script_path)
    ...
    try:
        subprocess.Popen(
            [
                ...
                "bash",
                "-c",
                f"cd scripts/ && {script_path} {bssid} {channel}",
            ]
        )
    except subprocess.CalledProcessError:
        raise HTTPException(status_code=500, detail="Error executing the script")
